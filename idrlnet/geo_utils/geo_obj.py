...
class Line1D(Geometry1D):
    def __init__(self, point_1, point_2):
        ...
        ranges = {none: (0, 1)}
        edge_1 = Edge(functions={"x": point_1, "normal_x": -1}, area=1.0, ranges=ranges)
        edge_2 = Edge(functions={"x": point_2, "normal_x": 1}, area=1.0, ranges=ranges)
...
class Line(Geometry2D):
    def __init__(self, point_1, point_2, normal=1):
        ...
        self.bounds = {
            "x": (min(point_1[0], point_2[0]), max(point_1[0], point_2[0])),
    ...
    def __init__(self, center=(0, 0.5), radius=0.5):
        ...
        edge_3 = Edge(
            functions={
                "x": center[0]
                - radius / 2
                + radius / math.sqrt(2) * cos(math.pi / 4 * 5 - theta),
                "y": center[1]
                + radius / 2
                + radius / math.sqrt(2) * sin(math.pi / 4 * 5 - theta),
        ...
        edge_4 = Edge(
            functions={
                "x": center[0]
                + radius / 2
                + radius / math.sqrt(2) * cos(math.pi / 4 * 3 - theta),
                "y": center[1]
                + radius / 2
                + radius / math.sqrt(2) * sin(math.pi / 4 * 3 - theta),
        ...
        part1 = Heaviside(Abs(x) + y - 1) * (
            sqrt((Abs(x) - 0.25) ** 2 + (y - 0.75) ** 2) - math.sqrt(2) / 4
        )
        part_i = 0.5 * Max(Abs(x) + y, 0)
        part2 = (
            (1 - Heaviside(Abs(x) + y - 1))
            * sign(Abs(x) - y)
            * Min(
                sqrt(Abs(x) ** 2 + (y - 1) ** 2),
                sqrt((Abs(x) - part_i) ** 2 + (y - part_i) ** 2),
            )
        )
        ...
        self.bounds = {
            ...
            "y": (
                center[1] - radius,
                center[1] + 0.5 * radius + 0.5 * math.sqrt(2) * radius,
            ),
        }
...
class Triangle(Geometry2D):
    def __init__(self, p0, p1, p2):
        ...
        v = Min(
            s * (v0.dot(N.i) * e0.dot(N.j) - v0.dot(N.j) * e0.dot(N.i)),
        ...
        self.bounds = {
            "x": (min(p0[0], p1[0], p2[0]), max(p0[0], p1[0], p2[0])),
    ...
    def __init__(self, points):
        ...
        def _sdf(x: np.ndarray, y: np.ndarray, **kwargs):
            ...
            for i in range(len(v)):
                ...
                b = w - e * np.clip(
                    (w * e).sum(axis=1, keepdims=True) / (e * e).sum(), 0, 1
                )
                ...
                inverse_idx2 = np.all(
                    [
                        ...
                        np.logical_not(cond2),
                        np.logical_not(cond3),
                    ],
                    axis=0,
                )
        ...
        for i, _ in enumerate(points):
            ...
            edge = Edge(
                functions={
                    "x": points[i - 1][0] - t * (points[i - 1][0] - points[i][0]),
        ...
        _p1 = next(_p)
        _p2 = next(_p)
        self.bounds = {"x": (min(_p1), max(_p1)), "y": (min(_p2), max(_p2))}
